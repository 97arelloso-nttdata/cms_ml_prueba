...
fix-lint: ## fix lint issues using autoflake, autopep8, and isort
	find cms_ml -name '*.py' | xargs autoflake --in-place --remove-all-unused-imports --remove-unused-variables
	autopep8 --in-place --recursive --aggressive cms_ml
	isort --apply --atomic --recursive cms_ml
...
coverage: ## check code coverage quickly with the default Python
	coverage run --source cms_ml -m pytest
	coverage report -m
	coverage html
...
docs: clean-docs ## generate Sphinx HTML documentation, including API docs
	sphinx-apidoc --module-first --separate -T -o docs/api/ cms_ml
	$(MAKE) -C docs html
...
.PHONY: docker-build
docker-build:
	docker build -f docker/Dockerfile -t cms-ml .

.PHONY: docker-push
docker-push: docker-build
	@$(eval VERSION := $(shell python -c 'import cms_ml; print(cms_ml.__version__)'))
	docker tag cms-ml docker.pkg.github.com/signals-dev/cms-ml/cms-ml:$(VERSION)
	docker push docker.pkg.github.com/signals-dev/cms-ml/cms-ml:$(VERSION)
	docker tag cms-ml docker.pkg.github.com/signals-dev/cms-ml/cms-ml
	docker push docker.pkg.github.com/signals-dev/cms-ml/cms-ml
